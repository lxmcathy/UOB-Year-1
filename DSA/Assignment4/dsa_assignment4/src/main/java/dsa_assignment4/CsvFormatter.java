 ...
 * <li>Columns all have type String: no special interpretation is made to treat
 * a string of digits as a number and comparisons between rows based on a
 * particular column always uses only string comparison, never numeric comparisons</li>
 * </ul>
...
public class CsvFormatter
{
	...
	/**
	 * Reads and returns a CSV row from an open scanner
	 * 
	 * @param scanner
	 *            The open scanner to read from
	 * @return The array of strings which form the fields of the row, or null
	 * 				if there are no more rows to read in the file
	 * @throws Exception
	 *             if scanner is not an open <code>Scanner</code> object or from
